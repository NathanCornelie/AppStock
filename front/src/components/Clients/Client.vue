...
<template >
  <v-dialog style="max-width: 1000px" v-model="clientStore.isClientModale"  persistent template >
    <v-card >
      <div class="d-flex align-center justify-lg-space-between pa-3"> <h2>Clients</h2> <v-btn @click="changeIsCreate()"><p v-if="isCreate">All </p><p v-else> Create</p></v-btn></div>

      <div v-if="isCreate"><CreateClient v-on:changeIsCreated="changeIsCreate" v-on:snackClientCreated="snackClientCreated"/></div>
